...
AWS Step Functions（以下、SFN）を運用していると、Datadogなどで「実行が開始からどれくらい経過しているか」をリアルタイムに監視したくなる場面があります。

ただし、SFNが標準で提供する実行時間の情報は、基本的に**実行が完了してから確定する“結果の所要時間”**です。
そのため、実行中のSFNに対して「開始から何秒経過しているか」を標準メトリクスだけで取得することはできません。

そのため、あるステップで想定外のリトライが続いて実行が長時間化していても、完了するまで気づけず、検知が遅れてユーザー影響が大きくなることがあります。
...
ここでは、カスタムメトリクスの送信方法（DogStatsD、HTTP API、カスタム Agent チェックなど）が詳細に解説されています。メトリクス名とタグの組み合わせによる識別方法も記載。​
...
- **最大経過秒だけ送る**: 実行中のSFNが複数ある場合「いちばん長く動いている1件」の経過秒だけを送ります。目的が“滞留の早期検知”なので最大値だけ見れば十分で、全実行分を送る方式に比べて送信量(＝Datadog側のコスト)やHTTP 429（Too Many Requests）のリスクを増やしにくくなります。
